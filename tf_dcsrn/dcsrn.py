...
import util
from layers import (weight_variable, weight_variable_devonc, pixel_wise_softmax_2, cross_entropy)
...
def create_conv_net(x, keep_prob, channels, n_class, layers=3, features_root=16, filter_size=3, pool_size=2, summaries=True):
    ...
    logging.info("Layers {layers}, features {features}, filter size {filter_size}x{filter_size}".format(layers=layers, features=features_root, filter_size=filter_size))
    ...
    for layer in range(0, layers):
        ...
        if layer == 0:
            ...
            in_offset1 = ConvOffset2D(features//2, init_normal_stddev = stddev)(in_node)
            conv1 = conv2d(in_offset1, w1, keep_prob)
        tmp_h_conv = tf.nn.relu(conv1 + b1)
        in_offset2 = ConvOffset2D(features, init_normal_stddev = stddev)(tmp_h_conv)
        conv2 = conv2d(in_offset2, w2, keep_prob)
    ...
    for layer in range(layers-2, -1, -1):
        ...
        #in_offset1 = ConvOffset2D(features, init_normal_stddev = stddev)(h_deconv_concat)
        conv1 = conv2d(h_deconv_concat, w1, keep_prob)
        h_conv = tf.nn.relu(conv1 + b1)
        #in_offset2 = ConvOffset2D(features//2, init_normal_stddev = stddev)(h_conv)
